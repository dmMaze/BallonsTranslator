...
* <details>
  ...
  **翻译历史**
  ...
  - 启用 **Use Prior Translations as Context** 后，`LLMTranslator` 会参考之前已完成的页面，有助于统一人名、术语和语气。继续运行和选定范围也可使用范围之前符合条件的页面。
  - **Prior Context Token Budget** 控制加入多少较早的译文，并优先保留较新的页面。当前页面、指令、术语表和生成回复还需要额外的上下文空间。默认值为 `4096`。
  - 较大预算可提供更多剧情上下文并减少旧页淘汰，但会发送更多输入，可能需要更长时间。本地模型还可能显著增加内存/显存占用。建议从默认值开始，仅在确有帮助时提高；模型上下文上限的约 70% 可作为实用上限（128K 模型约为 `90000`）。
