...
* <details>
  ...
  **翻译历史**
  ...
  - 启用 **Use Prior Translations as Context** 后，`LLMTranslator` 会使用之前已完成页面作为示例。继续运行和选定范围也可使用范围之前符合条件的页面，同一次运行中新完成的页面可供后续页面使用。
  - **Prior Context Token Budget** 只限制历史示例。程序优先保留较新的完整页面；当前页面、系统提示词、术语表和生成回复会额外占用上下文。
  - 默认值 `4096` 较为保守。如果已知模型标称的上下文窗口，可将约 70% 作为实用上限（128K 模型约为 `90000`），而非必须采用的起始值。仅在需要时提高：更多上下文会增加 token、时间和费用；本地推理还可能因 KV 缓存而显著增加内存/显存占用并降低速度。服务商也可能设置更低的限制。
  - 手动修改原文或译文不会改变页面完成状态。已保存译文不再对应原文时，请重新翻译该页面。
  - OpenAI、DeepSeek 等支持提示词缓存的服务商可复用连续页面间逐步增长的公共前缀。成批淘汰历史页面，或修改配置文件、术语表及其他前置提示内容时会重置复用，之后会重新增长。缓存取决于服务商，不会改变翻译行为。
