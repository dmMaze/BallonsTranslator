...
* <details>
  ...
  **翻译历史**
  ...
  - `LLMTranslator` 可以把之前已完成页面的原文/译文对作为示例。在运行对话框的“翻译”区域启用 **Use Prior Translations as Context**。
  - **Prior Context Token Budget** 只是历史示例的软限制。程序会优先选择较新的完整页面，再按页面顺序插入。当前页面、系统提示词、术语表和生成的回复也会占用此预算之外的上下文。
  - 继续运行或只运行选定页码范围时，仍可能使用所选范围之前符合条件的页面。同一次运行中成功翻译的页面可以立即作为后续页面的上下文。
  - 手动修改原文或译文不会自动改变页面的完成状态。修改原文后，如果已保存的译文不再对应，请重新翻译该页面。
  - 更多上下文可能提高一致性，但也会增加输入 token、请求时间和费用。建议先使用较小预算，仅在需要时提高。
  - 使用 OpenAI、DeepSeek 等支持提示词缓存的服务商或模型时，逐页增长的历史上下文通常能在连续页面间保持完全一致的公共前缀，因此缓存命中可降低输入成本和延迟。当历史预算淘汰较早页面，或配置文件、术语表及其他前置提示内容发生变化时，缓存可能失效；缓存支持取决于服务商，不会改变翻译行为。

  **可复用术语表**

  - 在运行对话框中设置 **Glossary File**，可使用 UTF-8 编码的 `.json`、`.txt` 或 `.tsv` 文件。文件只会被读取，并可在多个项目间复用。
  - **Matching** 只发送原词出现在相关页面中的条目；**All** 会发送全部条目，可能明显增加 token 用量。
  - 支持以下格式：
