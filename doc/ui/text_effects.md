...
QTextDocument + settled SceneTextLayout
  ...
  -> canonical foreground or Gradient Overlay, unless Hollow
...
entries are allowed. Hollow and Gradient Overlay are unique.
...
| Gradient Overlay | Foreground, unique | Recolors the canonical rich foreground without changing the alpha used by Stroke or generated effects. |
| Hollow | Foreground modifier, unique | Removes the canonical face, Gradient Overlay, and interior phase while retaining Stroke and exterior output. It is a toggle, not an independent painted layer. |

Stroke, Shadow, and Glow use either `SolidPaint` or `LinearGradientPaint`.
Gradient Overlay always uses a linear gradient. A gradient contains two to 32
...
zero-extent values must not allocate effect surfaces unnecessarily.
...
5. Paint canonical rich foreground or its Gradient Overlay unless Hollow is
   active.
...
multiply coverage exactly once. Gradient Overlay instead blends paint over the
foreground while preserving its alpha. Preserve straight-RGBA rounding at the
...
| Positioned or gradient Stroke, Shadow/Glow, Gradient Overlay, Hollow, or active mask | One bounded completed effect surface, full or tiled |
...
3. Define its external reach and whether Hollow, Stroke-inclusive exterior
   alpha, canonical alpha, Gradient Overlay, and the block mask affect it.
