...
* <details>
  ...
  **번역 기록**
  ...
  - `LLMTranslator`는 이전에 완료된 페이지의 원문/번역문 쌍을 예시로 사용할 수 있습니다. 실행 대화 상자의 번역 섹션에서 **Use Prior Translations as Context**를 활성화하세요.
  - **Prior Context Token Budget**은 번역 기록 예시에만 적용되는 유연한 제한입니다. 최근에 완료된 페이지가 우선 선택되며, 선택된 기록 페이지는 페이지 순서대로 삽입됩니다. 현재 입력, 시스템 지침, 용어집, 생성된 응답도 이 예산 외의 문맥을 추가로 사용합니다.
  - 이어서 실행하거나 선택한 범위만 번역할 때에도 해당 범위 이전의 적합한 페이지를 사용할 수 있습니다. 같은 실행 중에 번역이 완료된 페이지는 이후 페이지의 문맥으로 사용할 수 있습니다.
  - 원문이나 번역문을 수동으로 편집해도 페이지의 완료 상태가 자동으로 바뀌지는 않습니다. 원문을 변경하여 저장된 번역문과 더 이상 일치하지 않으면 해당 페이지를 다시 번역하세요.
  - 문맥이 많을수록 일관성이 향상될 수 있지만 입력 토큰 사용량, 요청 시간, 비용도 증가합니다. 적당한 예산으로 시작하고 필요할 때만 늘리세요.
  - OpenAI 및 DeepSeek처럼 프롬프트 캐싱을 지원하는 공급자와 모델에서는 기록이 늘어나는 동안 연속된 페이지 간에 정확히 같은 공통 접두사가 대체로 유지되므로 캐시 적중을 통해 입력 비용과 지연 시간을 줄일 수 있습니다. 기록 예산으로 인해 오래된 페이지가 제외되거나 프로필, 용어집 또는 앞부분의 다른 프롬프트 내용이 변경되면 캐시가 재사용되지 않을 수 있습니다. 캐싱 지원 여부는 공급자에 따라 다르며 번역 동작 자체에는 영향을 주지 않습니다.
