...
* <details>
  ...
  **번역 기록**
  ...
  - **Use Prior Translations as Context**를 활성화하면 `LLMTranslator`가 이전에 완료된 페이지를 예시로 사용합니다. 이어서 실행하거나 선택한 범위 이전의 적합한 페이지도 사용할 수 있으며, 새로 완료된 페이지는 이후 페이지의 문맥이 됩니다.
  - **Prior Context Token Budget**은 기록만 제한합니다. 최근 완료된 페이지를 우선하며 현재 페이지, 시스템 지침, 용어집, 생성 응답은 추가 문맥을 사용합니다.
  - 기본값 `4096`은 보수적입니다. 모델이 공개한 컨텍스트 창을 알면 약 70%를 실용적인 상한으로 사용할 수 있습니다(128K는 `90000`). 필수 시작값은 아닙니다. 필요할 때만 늘리세요. 문맥이 많아지면 토큰, 시간, 비용이 늘고, 로컬 추론에서는 KV 캐시 때문에 RAM/VRAM 사용량이 크게 증가하고 느려질 수 있습니다. 공급자가 더 낮은 제한을 적용할 수도 있습니다.
  - 원문이나 번역문을 수동으로 편집해도 완료 상태는 바뀌지 않습니다. 저장된 번역문이 원문과 맞지 않으면 다시 번역하세요.
  - OpenAI와 DeepSeek 같은 프롬프트 캐싱 공급자는 연속 페이지 사이에서 늘어나는 공통 접두사를 재사용할 수 있습니다. 기록 일괄 제거 또는 프로필, 용어집, 앞선 프롬프트 내용 변경 시 재사용이 초기화된 뒤 다시 증가합니다. 캐싱은 공급자에 따라 달라지며 번역 동작은 바뀌지 않습니다.
