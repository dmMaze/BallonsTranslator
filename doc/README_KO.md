...
* <details>
  <summary><i>시각적 문맥, 페이지별 요약, 프로젝트 전체 요약을 지원하는 LLM 번역</i></summary>

  **비전**

  활성화하면 시각 인식 기능이 있는 모델이 이미지를 번역 문맥으로 활용할 수 있습니다.

  **요약 및 메모리**

  페이지별 요약에는 등장인물과 관계, 배경, 주요 사건, 화자를 파악할 수 있는 단서, 지칭 대상이 아직 불분명한 표현 등 번역에 필요한 정보를 간략히 기록합니다. 메모리는 누적된 페이지 요약을 바탕으로 프로젝트 전체의 정보를 압축한 기록입니다. 오래된 요약이 더 이상 문맥 예산에 들어가지 않거나 마지막 페이지의 처리가 끝나면, 프로그램은 텍스트만 포함된 별도의 요청을 보내 해당 요약을 기존 메모리에 통합합니다. 이후 번역에서는 이 압축된 기록을 안정적인 문맥으로 재사용합니다.
