...
* <details>
  ...
  **翻訳履歴**
  ...
  - **Use Prior Translations as Context** を有効にすると、`LLMTranslator` は以前に完了したページを例として利用します。続行実行や選択範囲より前の対象ページも利用でき、新しく完了したページは後続ページのコンテキストになります。
  - **Prior Context Token Budget** は履歴だけを制限します。新しい完了ページが優先され、現在のページ、システム指示、用語集、生成応答は追加のコンテキストを使用します。
  - デフォルトの `4096` は控えめです。モデルの公表コンテキストウィンドウが分かる場合、約 70% が実用上の上限です（128K なら `90000`）。必須の開始値ではありません。必要な場合だけ増やしてください。コンテキストを増やすとトークン、時間、料金が増え、ローカル推論では KV キャッシュにより RAM/VRAM 使用量が大幅に増えて低速になる場合もあります。プロバイダーの上限がより低い場合もあります。
  - 原文や訳文を手動で編集しても完了状態は変わりません。保存済みの訳文が原文と一致しなくなった場合は再翻訳してください。
  - OpenAI や DeepSeek などのプロンプトキャッシュ対応プロバイダーは、連続ページ間で増える共通プレフィックスを再利用できます。履歴の一括削除やプロファイル、用語集、前方のプロンプト変更で再利用はリセットされ、その後また増加します。キャッシュはプロバイダー依存で、翻訳動作は変わりません。
