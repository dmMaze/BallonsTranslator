...
* <details>
  ...
  **翻訳履歴**
  ...
  - **Translation history** を有効にすると、`LLMTranslator` は以前に完了したページを例として参照します。人名、用語、口調の一貫性向上に役立ちます。続行実行や選択範囲でも、対象となる以前のページを利用できます。
  - **History limit (tokens)** は、以前の訳文をどれだけ含めるかを制御し、新しいページを優先します。現在のページ、指示、用語集、生成応答には追加の空きが必要です。デフォルトは `4096` です。
  ...
  | History limit (tokens) | 保持される履歴の目安（ページ） | 履歴なしと比較した推定総費用 |
  |---:|---:|---:|
  | `2048` | 3–4 | 1.65× |
  | `4096` | 6–9 | 1.79× |
  | `8192` | 12–19 | 2.10× |
