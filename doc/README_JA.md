...
* <details>
  <summary><i>視覚コンテキスト、ページごとの要約、プロジェクト全体の要約に対応した LLM 翻訳</i></summary>

  **ビジョン**

  有効にすると、画像を認識できるモデルが画像を翻訳のコンテキストとして利用できます。

  **要約とメモリ**

  ページごとの要約には、登場人物とその関係、場面、重要な出来事、発話者を特定する手掛かり、参照先が不明な指示語など、翻訳に関わる情報を簡潔に記録します。メモリは、蓄積されたページ要約をもとに作成する、プロジェクト全体の情報を凝縮した記録です。古い要約がコンテキスト予算に収まらなくなった場合、または最終ページの処理が完了した場合、プログラムはテキストのみの独立したリクエストを送り、それらの要約を既存のメモリに統合します。その後の翻訳では、この凝縮した記録を安定したコンテキストとして再利用します。
