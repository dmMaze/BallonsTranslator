...
* <details>
  ...
  **翻訳履歴**
  ...
  - `LLMTranslator` は、以前に完了したページの原文と訳文の組を例として利用できます。実行ダイアログの翻訳セクションで **Use Prior Translations as Context** を有効にしてください。
  - **Prior Context Token Budget** は履歴例だけに適用されるソフト上限です。新しい完了ページが優先され、選択された履歴ページはページ順に挿入されます。現在のページ、システム指示、用語集、生成される応答も、この予算とは別にコンテキストを消費します。
  - 続行実行や選択範囲の実行でも、範囲より前にある条件を満たすページが使用される場合があります。同じ実行中に翻訳が完了したページは、後続ページのコンテキストとして利用できます。
  - 原文や訳文を手動で編集しても、ページの完了状態は自動では変わりません。原文を変更して保存済みの訳文と一致しなくなった場合は、そのページを再翻訳してください。
  - コンテキストを増やすと一貫性が向上する可能性がありますが、入力トークン、処理時間、料金も増加します。小さな予算から始め、必要な場合だけ増やしてください。
  - OpenAI や DeepSeek など、プロンプトキャッシュに対応するプロバイダーやモデルでは、履歴が増えている間は連続するページ間で完全に一致する共通プレフィックスが通常維持されるため、キャッシュヒットによって入力コストと待ち時間が減る場合があります。履歴予算によって古いページが除外された場合や、プロファイル、用語集、その他の前方のプロンプト内容が変わった場合は、キャッシュが再利用されないことがあります。キャッシュ対応はプロバイダーに依存し、翻訳の動作自体は変わりません。
