...
* <details>
  ...
  **История переводов**
  ...
  - Включите **Use Prior Translations as Context**, чтобы `LLMTranslator` использовал завершённые предыдущие страницы как примеры. Продолжение и выбранный диапазон могут учитывать подходящие более ранние страницы, а новые завершённые страницы становятся контекстом для следующих.
  - **Prior Context Token Budget** ограничивает только историю. Приоритет имеют новые завершённые страницы; текущая страница, системные инструкции, глоссарий и ответ используют дополнительный контекст.
  - Значение `4096` по умолчанию консервативно. Если заявленное окно модели известно, практический верхний предел — около 70% (`90000` для 128K), а не обязательное начальное значение. Увеличивайте его только при необходимости: больший контекст требует токенов, времени и денег; при локальном запуске кэш KV также может значительно увеличить расход RAM/VRAM и замедлить работу. Провайдер может задавать меньшие лимиты.
  - Ручное изменение исходного текста или перевода не меняет состояние завершения. Переведите страницу заново, если сохранённый перевод больше не соответствует исходному тексту.
  - Провайдеры с кэшированием промптов, например OpenAI и DeepSeek, могут повторно использовать растущий общий префикс последовательных страниц. Групповое удаление истории и изменения профиля, глоссария или предшествующего промпта сбрасывают повторное использование, после чего оно снова растёт. Кэширование зависит от провайдера и не меняет перевод.
