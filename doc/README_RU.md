...
* <details>
  <summary><i>Контекстный перевод с помощью LLM</i></summary>

  **История переводов**

  - Включите **Translation history**, чтобы показать `LLMTranslator` примеры с завершённых предыдущих страниц. Это помогает сохранить единообразие имён, терминов и тона. Продолжение и выбранные диапазоны также могут использовать подходящие более ранние страницы.
  - **History limit (tokens)** определяет объём предыдущего переведённого текста, отдавая приоритет новым страницам. Для текущей страницы, инструкций, глоссария и ответа нужно дополнительное место. Значение по умолчанию — `4096`.
  ...
  | History limit (tokens) | Оценка сохраняемой истории (страницы) | Оценка общей стоимости относительно режима без истории |
  |---:|---:|---:|
  | `2048` | 3–4 | 1.65× |
  | `4096` | 6–9 | 1.79× |
  | `8192` | 12–19 | 2.10× |
