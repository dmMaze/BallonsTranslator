...
* <details>
  ...
  **История переводов**
  ...
  - Включите **Use Prior Translations as Context**, чтобы показать `LLMTranslator` примеры с завершённых предыдущих страниц. Это помогает сохранить единообразие имён, терминов и тона. Продолжение и выбранные диапазоны также могут использовать подходящие более ранние страницы.
  - **Prior Context Token Budget** определяет объём предыдущего переведённого текста, отдавая приоритет новым страницам. Для текущей страницы, инструкций, глоссария и ответа нужно дополнительное место. Значение по умолчанию — `4096`.
  - Больший бюджет даёт больше контекста сюжета и реже удаляет старые страницы, но отправляет больше текста и может работать медленнее. Локальным моделям также может потребоваться значительно больше RAM/VRAM. Начните со значения по умолчанию и увеличивайте его только при заметной пользе; разумный верхний предел — около 70% окна модели (`90000` для 128K).
