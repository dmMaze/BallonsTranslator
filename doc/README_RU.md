...
* <details>
  <summary><i>LLM-перевод с визуальным контекстом, сводками по страницам и общей сводкой проекта</i></summary>

  **Зрение**

  При включении этой функции модели с поддержкой зрения могут использовать изображения как контекст для перевода.

  **Сводки и память**

  Сводка страницы кратко фиксирует сведения, важные для перевода: персонажей и их отношения, обстановку, ключевые события, подсказки о том, кто говорит, и упоминания, для которых ещё неясно, к кому или чему они относятся. Память — это сжатая запись обо всём проекте, составленная из накопленных сводок страниц. Когда старые сводки перестают помещаться в бюджет контекста или завершается обработка последней страницы, программа отправляет отдельный запрос, содержащий только текст, чтобы объединить эти сводки с существующей памятью. При последующих переводах эта сжатая запись повторно используется как стабильный контекст.
