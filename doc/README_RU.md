...
* <details>
  ...
  **История переводов**
  ...
  - `LLMTranslator` может использовать пары исходного и переведённого текста с ранее завершённых страниц как примеры. Включите **Use Prior Translations as Context** в разделе перевода окна запуска.
  - **Prior Context Token Budget** — мягкий лимит только для исторических примеров. Приоритет отдаётся более новым завершённым страницам, после чего они добавляются в порядке страниц. Текущая страница, системные инструкции, глоссарий и сгенерированный ответ также занимают контекст сверх этого бюджета.
  - При продолжении или переводе выбранного диапазона могут использоваться подходящие страницы перед этим диапазоном. Успешно переведённая страница становится доступной как контекст для следующих страниц в том же запуске.
  - Ручное изменение исходного текста или перевода не меняет состояние завершения страницы автоматически. Если после изменения исходного текста сохранённый перевод больше ему не соответствует, переведите страницу заново.
  - Дополнительный контекст может улучшить единообразие, но увеличивает расход входных токенов, время запроса и стоимость. Начните с небольшого бюджета и увеличивайте его только при необходимости.
  - При использовании провайдера или модели с кэшированием промптов, например OpenAI или DeepSeek, растущая история обычно сохраняет точный общий префикс между последовательными страницами, поэтому попадания в кэш могут снизить стоимость входных данных и задержку. Повторное использование кэша может прекратиться, когда из-за бюджета истории удаляются старые страницы либо меняются профиль, глоссарий или другой предшествующий контент промпта; поддержка кэширования зависит от провайдера и не влияет на поведение перевода.
