...
* Редактирование изображенийПоддержка редактирования масок и закраски (что-то вроде инструмента точечной лечебной кисти в PS)
...
```
### Смена ветки на dev(если хотите опробовать бета версию)
```bash
git switch dev
```
...
python launch.py
```
## Автоустновщик(с веткой dev так, как она пока стабильнее)

Тут все просто, переходите в репозиторий, качаете файл и следуете инструкции.

[Ссылка](https://github.com/bropines/Ballon-translator-portable)
