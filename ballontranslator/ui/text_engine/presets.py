...
def _drop_event_position(event) -> QPoint:
...
class TextStyleLabel(Widget):
    ...
    @classmethod
    def _is_text_style_drag(cls, event) -> bool:
        return (
            isinstance(event.source(), TextStyleLabel)
...
class TextStylePresetPanel(PanelArea):
    ...
    def _moveStyleLabelToIndex(self, source: TextStyleLabel, target_index: int) -> None:
        ...
        # The label widgets and text_styles must always share their ordering.
        self.flayout.takeAt(source_index)
        self.flayout.insertWidget(target_index, source)
        text_styles.insert(target_index, text_styles.pop(source_index))
        self.flayout.invalidate()
        self.resizeToContent()
        save_text_styles()

    def _style_label_index(self, label: TextStyleLabel):
    ...
    def _reorderStyleAtPosition(self, source: TextStyleLabel, position: QPoint) -> None:
        ...
        for index in range(self.count()):
            label_item = self.flayout.itemAt(index)
            label = label_item.widget() if label_item is not None else None
            if label is None:
                continue
            geometry = label.geometry()
            if position.y() < geometry.center().y():
                insert_index = index
                break
            if geometry.top() <= position.y() <= geometry.bottom():
                insert_index = index + int(position.x() >= geometry.center().x())
