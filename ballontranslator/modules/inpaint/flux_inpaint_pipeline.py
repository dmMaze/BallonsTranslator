...
from diffusers.utils import is_torch_xla_available, logging, replace_example_docstring, BaseOutput
...
class Flux2KleinInpaintPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    ...
    def prepare_image_latents(
        ...
        for image in images:
            if isinstance(image, np.ndarray):
                image = rearrange(image.astype(np.float32), 'h w c -> c h w')[None]
    ...
    def __call__(
        ...
        noise, latent_ids = self.prepare_latents(
            batch_size=batch_size * num_images_per_prompt,
        ...
        mask_tensor = torch.from_numpy((mask > 0.5).astype(np.float32))[None, None]
        latent_mask = self.prepare_mask_latents(
            mask_tensor,
            batch_size,
            num_images_per_prompt,
            height,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents = (1 - latent_mask) * init_latents_proper + latent_mask * latents
        ...
        image = self.decode_latents((height, width), latents=latents, latent_ids=latent_ids, output_type=output_type)
    ...
    def decode_latents(self, output_size, latents, latent_ids, output_type='pil'):
